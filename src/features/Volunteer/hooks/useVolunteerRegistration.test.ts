...
describe('useVolunteerRegistration Hook', () => {
  ...
  it('initializes with the default roster', () => {
    ...
    expect(result.current.stats.totalRegistered).toBe(7);
  });
  ...
  it('adds a volunteer registration and persists to localStorage', () => {
    ...
    expect(result.current.registeredVolunteers[7].fullName).toBe('John Smith');
    expect(result.current.registeredVolunteers[7].phone).toBe('+234 803 123 4567');
    expect(result.current.registeredVolunteers[7].email).toBe('john@gmail.com');
    ...
    expect(stored[7].fullName).toBe('John Smith');
  });
  ...
  it('clears all volunteer registrations', () => {
    ...
    expect(result.current.registeredVolunteers.length).toBe(7);
